...
const Enquiries = () => {
  ...
  const filteredData = enq.filter((item) => {
    const hasValidService =
      item.srv_id && item.srv_id[0] && item.srv_id[0].srv_id;
    const hasValidStartDate = hasValidService && item.srv_id[0].start_date;

    const isDateMatch =
      selectedDate === "" ||
      (hasValidStartDate && item.srv_id[0].start_date.includes(selectedDate));
    const isServiceMatch =
      selectedService === "" ||
      (hasValidService &&
        item.srv_id[0].srv_id.service_title
          .toLowerCase()
          .includes(selectedService.toLowerCase()));
    const isCallerStatusMatch =
      selectedCallerStatus === "" ||
      selectedCallerStatus === 5 ||
      item.patient_service_status === selectedCallerStatus;
  ...
  return (
    <>
      <Navbar />
      {/* <Header /> */}
      <Box sx={{ flexGrow: 1, mt: 14, ml: 1, mr: 1, mb: 4 }}>
        <Stack
                    ...
                    <TableRow key={row.eve_id}>
                      <EnquiryCard
                        ...
                        sx={{
                          ...
                          cursor: "pointer",
                          "&:hover": {
                            borderLeft: "4px solid rgb(162, 217, 225)",
                            borderRight: "4px solid rgb(162, 217, 225)",
                            transform: "scale(1)",
                          },
                        }}
                      >
                        ...
                        <CardContent style={{ flex: 1 }}>
                          <Typography variant="body2">
                            {formatDate(
                              row.srv_id[0] ? row.srv_id[0].start_date : "-"
                            )}
                          </Typography>
                        </CardContent>
                        <CardContent style={{ flex: 0.8 }}>
                          <Typography variant="body2">
                            {row.agg_sp_pt_id
                              ? row.agg_sp_pt_id.prof_zone_id?.Name
                              : "-"}
                          </Typography>
                        </CardContent>
                        <CardContent style={{ flex: 0.5 }}>
                        ...
                        <CardContent style={{ flex: 1 }}>
                          <Button
                            ...
                            sx={{
                              ...
                              fontSize: "14px",
                              fontWeight: 500,
                              borderColor: "rgb(186, 241, 248)",
                              color: "#0097A7", // deeper teal text
                              "&:hover": {
                                borderColor: "rgb(186, 241, 248)",
                                backgroundColor: "rgb(186, 241, 248)",
                                color: "#000",
                              },
                            }}
                          >
