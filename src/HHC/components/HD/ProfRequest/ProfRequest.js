...
const EnquiryCard = styled(Card)({
  ...
  "&:hover": {
    ...
    cursor: "pointer",
    // boxShadow: "4px 4px 10px 7px rgba(135, 135, 135, 0.15)",
  },
});
...
const ProfRequest = () => {
    ...
    if (selectedRequestID && selectedRequestbyID) {
      try {
        const res = await fetch(
          `${port}/web/get_reschedule_cancle_request/${selectedRequestID}/${selectedRequestbyID}/`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
  ...
  function findPreFollowupRecords(eveId) {
    const matchingRecords = profRequest.find(
      (record) => record.eve_id === eveId
    );
  ...
  async function handleRejectSubmit(event) {
    ...
    try {
      const response = await fetch(
        `${port}/web/professional_request_rejection/${rejectReqID}/`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <>
      <Navbar />
      <Box sx={{ flexGrow: 1, mt: 13.6, ml: 1, mb: 2, mr: 1 }}>
        <Stack
        ...
        <TableContainer
          sx={{
            height:
              profRequest.length === 0 || profRequest.length < 5
                ? "60vh"
                : "default",
          }}
        >
          <Table>
            <TableHead>
              <TableRow>
                <EnquiryCard
                  style={{
                    background: "rgb(240, 244, 204)",
                    color: "#000000",
                    borderRadius: "8px 10px 0 0",
                  }}
                >
                  <CardContent
                    style={{ flex: 0.3, borderRight: "1px solid #FFFFFF" }}
                  >
                      ...
                      >
                        <EnquiryCard
                          sx={{
                            ...
                            cursor: "pointer",
                            "&:hover": {
                              borderLeft: "4px solid rgb(136 151 2)",
                              borderRight: "4px solid rgb(136 151 2)",
                              transform: "scale(1)",
                            },
                          }}
                          // sx={{ bgcolor: "#f1f1f1" }}
                        >
                          ...
                          <CardContent style={{ flex: 1 }}>
                            ...
                            {selectedRequestbyID === "1" ? (
                              ...
                              <Typography variant="body2">
                                {row.dtl_eve_id
                                  ? row.dtl_eve_id?.srv_prof_id?.prof_fullname
                                  : "-"}
                              </Typography>
                            )}
                          </CardContent>
                          <CardContent style={{ flex: 1 }}>
                            ...
                            {selectedRequestbyID === "1" ? (
                              ...
                              <Typography variant="body2">
                                +91{" "}
                                {row.dtl_eve_id
                                  ? row.dtl_eve_id?.srv_prof_id?.phone_no
                                  : "-"}
                              </Typography>
                            )}
                          </CardContent>
                          <CardContent style={{ flex: 1 }}>
                            <Typography variant="body2">
                              {row.epoc_id
                                ? row.epoc_id.srv_id?.service_title
                                : "-"}
                            </Typography>
                          </CardContent>
                          {selectedRequestbyID !== "1" && (
                            <>
                              <CardContent style={{ flex: 0.5 }}>
                                <Typography variant="body2">
                                  {row.dtl_eve_id
                                    ? row.dtl_eve_id?.agg_sp_dt_eve_poc_id
                                    : "-"}
                                </Typography>
                              </CardContent>
                              <CardContent style={{ flex: 1 }}>
                                <Typography variant="body2">
                                  {row.dtl_eve_id
                                    ? row.dtl_eve_id?.actual_StartDate_Time
                                    : "-"}
                                </Typography>
                              </CardContent>
                            </>
                          ...
                          <CardContent style={{ flex: 1 }}>
                            <div style={{ display: "flex" }}>
                              <Button
                                ...
                                onClick={() =>
                                  handleOpen(findPreFollowupRecords(row.eve_id))
                                }
                              >
                                Approve
                              </Button>
                              <Button
                                ...
                                onClick={() =>
                                  handleOpenReject(
                                    findPreFollowupRecords(row.eve_id)
                                  )
                                }
                              >
                                Reject
                            ...
                            >
                              ...
                              >
                                <AppBar
                                  ...
                                  style={{
                                    background:
                                      " linear-gradient(90deg, rgb(240, 244, 204)",
                                    width: "22.8rem",
                                    height: "3rem",
                                    marginTop: "-16px",
                                    marginLeft: "-32px",
                                ...
                                <Request
                                  ...
                                  selectedRequestbyID={selectedRequestbyID}
                                  srvId={srvId}
                                  reqID={rejectReqID}
                            ...
                            >
                              <Box
                                sx={{
                                  ...
                                  bgcolor: "background.paper",
                                  boxShadow: 24,
                                  p: 0, // Removed outer padding to make AppBar align fully
                                }}
                              >
                                {/* Header with AppBar */}
                                <AppBar
                                ...
                                <Box sx={{ px: 2, pt: 2, pb: 3 }}>
                                  ...
                                  <Button
                                    ...
                                    sx={{
                                      ...
                                      borderRadius: "12px",
                                      color: "#FFFFFF",
                                      backgroundColor: "rgb(100, 154, 141)",
                                    }}
                                  >
                                    Submit
                                  </Button>
