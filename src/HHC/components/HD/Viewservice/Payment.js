...
const Payment = ({ eveID, pay, ptnData, onClose, walletData }) => {
    ...
    async function handleCashPaymentSubmit(event) {
        ...
        if (Number(convCharge.no_of_serssion) < 1) {
            setOpenSnackbar(true);
            setSnackbarMessage('To proceed, please ensure at least one session is completed');
            setSnackbarSeverity('error');
            return;
        }

        if (Number(convCharge.amount_return) > 0) {
            setOpenSnackbar(true);
            setSnackbarMessage('Return amount should be 0');
            setSnackbarSeverity('error');
            return;
        }
        ...
        /// Changed Data
        // const finalAmount = Number(convCharge.total_cahrge);
        // const paidAmount = Number(dynamicAmountPaid);
        // if (finalAmount > 0 && paidAmount !== finalAmount) {
        //     setOpenSnackbar(true);
        //     setSnackbarMessage(`Amount Paid should be equal to ${convCharge.total_cahrge?.toFixed(2)}.`);
        //     setSnackbarSeverity('error');
        //     return;
        // }
        ...
        if (finalAmount <= 0) {
            setOpenSnackbar(true);
            setSnackbarMessage('Final amount must be greater than 0 to proceed.');
            setSnackbarSeverity('error');
            return;
        }
        ...
        const requestData = {
            eve_id: eveID,
            // Total_cost: convCharge.total_cahrge,
            Total_cost: pay.Total_Amount,
            paid_by: ptnData.name,
            // amount_paid: convCharge.total_cahrge || 0,
    ...
    async function handleOnlinePaymentSubmit(event) {
        ...
        const finalOrderAmount = isChecked
            ? Math.max((paymentData.amount_paid || dynamicAmountPaid) - walletAmountUsed, 0)
        ...
        const requestData = {
            ...
            orderAmount: Number(dynamicAmountPaid) || finalOrderAmount,
            Remaining_amount: 0,
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const errorMsg = errorData?.detail || 'Payment failed with status ' + response.status;

                if (response.status === 400) {
                    setSnackbarMessage(`To Redeem Wallet Amount Choose Offline Payment Mode`);
                } else {
                ...
                <Box sx={{ width: '100%', typography: 'body1', mt: 2, overflowY: 'scroll', height: '400px' }}>
                    <TabPanel value="1"
                        ...
                        <Grid container spacing={2}>
                            ...
                            {walletData &&
                                walletData["Amount in wallet is "] > 0 &&
                                Number(paymentData.Total_Amount) !== Number(dynamicAmountPaid) && (
                                    <Grid item lg={12} sm={12} xs={12}>
                                        <FormControlLabel
                                            control={
                                                <Checkbox
                            ...
                            {!(isChecked && Number(walletamout) === Number(paymentData.Total_Amount)) && (
                                <Grid item lg={12} sm={12} xs={12}>
                                    <TextField
                                        id="mode_id"
                                        name="mode_id"
                                    ...
                                    >
                                        {payMode.map((option) => (
                                            <MenuItem key={option.mode_id} value={option.mode_id} sx={{ fontSize: "14px" }}>
                                                {option.label}
                                            </MenuItem>
                                        ))}
                                    </TextField>
                                    ...
                                    <Grid item lg={12} sm={12} xs={12} style={{ marginTop: "10px" }}>
                                        <TextField
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const expectedAmount = Number(paymentData.Total_Amount) - Number(walletamout);

                                                if (raw === "") {
                                                    setDynamicAmountPaid("");
                                                    setAmountError("");
                                                    return;
                                                }
                                                ...
                                                // ✅ This block handles your request: remove error if exact amount is entered
                                                if (entered === expectedAmount) {
                                                    setDynamicAmountPaid(raw);
                                                    setAmountError("");
                                                    handleFieldChange("amount_paid", entered);
                                                    return;
                                                }

                                                if (entered > expectedAmount) {
                                                    setAmountError(`Amount cannot exceed ₹${expectedAmount}`);
                                                    return;
                                                }
                                            ...
                                            error={!!amountError && dynamicAmountPaid !== ""}
                                            helperText={
                                                dynamicAmountPaid !== ""
                                                    ? amountError || sessionError || `You can pay up to ₹${Number(paymentData.Total_Amount) - Number(walletamout)}`
                                                    : `Enter Amount ₹${Number(paymentData.Total_Amount) - Number(walletamout)}`
                                            }
                                            FormHelperTextProps={{
                                                sx: {
                                                    fontSize: '13px',
                        ...
                        <Grid container spacing={2}>
                                    ...
                                    <FormControlLabel
                                        control={
                                            <Checkbox
                                                ...
                                                onChange={handleCheckboxChange}
                                                color="primary"
                                                disabled={Number(dynamicAmountPaid) === Number(paymentData.Total_Amount)}
                                            />
                                        }
                                        label="Use Wallet Amount"
                                    />
                                ...
                                Number(paymentData.Total_Amount) !== Number(walletamout) && (
                                    <Grid item lg={12} sm={12} xs={12} style={{ marginTop: "10px" }}>
                                        <TextField
                                            ...
                                            onChange={(e) => {
                                                const raw = e.target.value;
                                                const expectedAmount = Number(paymentData.Total_Amount);
                            ...
                            <Grid item lg={12} sm={12} xs={12} style={{ marginTop: "2px" }}>
                                <Box component="form" sx={{ p: "4px 6px", alignItems: 'center', height: 'auto', backgroundColor: "#CBE3FF", boxShadow: "4px 4px 10px 7px rgba(135, 135, 135, 0.05)", borderRadius: "5px", }}>
                                    ...
                                    <Typography variant='body2'>
                                        Return Amount: {isChecked ? convCharge.amount_return?.toFixed(2) : convCharge.amount_return?.toFixed(2)}
                                    </Typography>
                                </Box>
                            </Grid>
