...
const Addservice = () => {
  ...
  console.log(enqToServicePtn, 'enqToServicePtn');

  const [enqToServiceSrv, setEnqToServiceSrv] = useState("");
  console.log(enqToServiceSrv, 'enqToServiceSrvenqToServiceSrv');
  const [enqToServicePay, setEnqToServicePay] = useState("");
  console.log(enqToServicePay, 'amount check');
  ...
  const [srvExtendSrvPurpId, setsrvExtendSrvPurpId] = useState(null);
  console.log(srvExtendSrvPurpId, "srvExtendSrvPurpId");
  const [values, setValues] = useState(enqToServiceSrv ? enqToServiceSrv.serivce_dates : []);

  console.log(values, 'valuesvaluesvalues');

  // const minDate = srvExtendSrv
  //   ? new Date(new Date(srvExtendSrv.end_date).getTime() + 24 * 60 * 60 * 1000)
  //   : new Date();
  ...
  // const minDate = srvExtendSrv
  //   ? new Date(Math.max(today.getTime(), new Date(srvExtendSrv.end_date).getTime()))
  //   : today;
  ...
  const minDate = srvExtendSrv
    ? new Date(new Date(srvExtendSrv.serivce_dates).getTime() - 24 * 60 * 60 * 1000)
    : today;
  ...
  console.log(gisAddress, 'gisAddress while enquiry to service');

  const [address, setAddress] = useState("");
  const [lat, setLat] = useState(null);
  const [long, setLong] = useState(null);
  console.log(lat, long, 'passing lat and long');
  ...
  const [selectedSubService, setSelectedSubService] = useState(
    ...
    // :
    srvExtendSrv.sub_srv_id ? srvExtendSrv.sub_srv_id.sub_srv_id : ""
  );
  //// HCA Questions ////
  ...
  const [startDate, setStartDate] = useState(
    enqToServiceSrv ? enqToServiceSrv.start_date : ""
  );
  const [endDate, setEndDate] = useState(
    enqToServiceSrv ? enqToServiceSrv.end_date : ""
  );
  ...
  // const [values, setValues] = useState(
  //   enqToServiceSrv ? enqToServiceSrv.serivce_dates : []
  // );
  ...
  const [startTime, setStartTime] = useState(
    enqToServiceSrv ? enqToServiceSrv.start_time : ""
  );
  const [endTime, setEndTime] = useState(
    enqToServiceSrv ? enqToServiceSrv.end_time : ""
  );
  ...
  console.log(calculatedAmount, 'calculatedAmountcalculatedAmount');

  const [selectedDiscountId, setSelectedDiscountId] = useState(5);
  const [discountValue, setDiscountValue] = useState(
    selectedDiscountId === 3 || selectedDiscountId === 4 ? 0 : null
  );
  ...
  console.log(eventIdService, 'eventIdServiceeventIdService');
  ...
  const handlePatientSelect = async (e) => {
    ...
    const patient = patientDetails.find(
      (patient) => patient.agg_sp_pt_id === selectedPatientId
    );
    ...
    try {
      const response = await fetch(
        `${port}/hhc_admin/patient_whitelist_api/${selectedPatientId}/`,
        {
          method: 'GET',
        }
      );
      ...
      if (response.status === 204) {
        console.log("No content available for this patient.");
        return; // ← Do nothing
      }

      if (response.status === 200) {
        const data = await response.json();
        console.log("Fetched patient API data:", data);
        setPatientData(data);

        // ✅ Check if blacklisted
        if (data.is_blocklisted === 1) {
          setOpenModal(true);
        }
      } else {
    ...
    if (selectedPatient) {
      ...
      // }
        ...
        const selectedConsultantData = consultant.find(
          (consult) => consult.doct_cons_id === value
        );
  ...
  const handlePhoneNumberChange = (e) => {
    ...
    if (!numericValue) {
      ...
      setErrors({
        ...errors,
        phoneNumber: "Mobile No should be a positive number",
      });
    } else {
      setPhoneNumberError("");
  ...
  const handleConsultNumberChange = (e) => {
    ...
    if (!numericValue) {
      ...
      setConsultErrors({
        ...errors,
        consultNo: "Mobile No should be a positive number",
      });
    } else {
      setConsultNoError("");
  ...
  const handleDropdownCoupon = (event) => {
    ...
    const selectedCouponCode = coupon.find(
      (coupon) => coupon.coupon_id === selectedValue
    );
    if (selectedCouponCode) {
      setCouponValue(selectedCouponCode.discount_value);
      console.log(
        "selectedCouponCode......",
        selectedCouponCode.discount_value
      );
  ...
  useEffect(() => {
    const getGender = async () => {
      try {
        ...
        const filteredGender = data.filter(
          (item) => item.gender_id === 1 || item.gender_id === 2
        );
  ...
  const handleSubServiceSelect = (event) => {
    ...
    const selectedSubService = subService.find(
      (item) => item.sub_srv_id === subServiceId
    );
    if (selectedSubService) {
  ...
  useEffect(() => {
    const getQuestion = async () => {
      ...
      if (selectedService) {
        ...
        try {
          const res = await fetch(
            `${port}/web/job_closure_questions/${selectedService}/`,
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
                "Content-Type": "application/json",
  ...
  const handleCheckboxChange = (event) => {
    ...
    if (event.target.checked) {
      ...
      setSelectedQuestions((prevChecked) =>
        prevChecked.filter((id) => id !== checkedId)
      );
    }
  };
    ...
    const calculateTotalAmount = async () => {
      ...
      if (getCost && effectiveDateCount) {
        ...
        try {
          ...
          const data = await res.json();
          console.log(
            "Calculated Service Cost datewise.....",
  ...
  useEffect(() => {
    const calculateDiscount = async () => {
      ...
      if (calculatedAmount && selectedDiscountId !== null) {
        ...
        try {
          let url;
          const ClearedDiscountValue = discountValue === "" ? 0 : discountValue;
          const ClearedCouponValue = couponValue === "" ? 0 : couponValue;
          ...
          if (selectedDiscountId === 3) {
            ...
            url = `${port}/web/calculate_discount_api/${selectedDiscountId}/${ClearedDiscountValue}/${calculatedAmount}`;
          } else if (selectedDiscountId === 6) {
            url = `${port}/web/calculate_discount_api/${selectedCouponType}/${ClearedCouponValue}/${calculatedAmount}`;
          ...
          console.log(
            "Calculated Amount with Discount.....",
            data.final_amount
          );
        } catch (error) {
          console.error(
            "Error fetching Calculated Amount with Discount:",
            error
          );
        }
      } else {
    ...
    const getPatientPreDetails = async () => {
      ...
      if (ptnID) {
        ...
        try {
          ...
          const eventId = data.Date?.eve_id;
          setEventIdService(eventId)
  ...
  useEffect(() => {
    const getFeedback = async () => {
      const ptnID = selectedPatientID || srvExtendPtnID;
      if (ptnID && eventIdService) {
        console.log("Selected Patient ID", ptnID);
        console.log("Using Event ID:", eventIdService); // Just to confirm
        try {
          const url = `${port}/web/patient_feedback_get_api/${eventIdService}/${ptnID}/`;
          const res = await fetch(url, {
            method: "GET",
            headers: {
              Authorization: `Bearer ${accessToken}`,
    ...
    getFeedback();
  }, [selectedPatientID, srvExtendPtnID, eventIdService]);

  const maxRating = 5;
  const generateRatingIcons = (rating) => {
    const filledIcons = Math.floor(rating);
    const remainder = rating % 1;
    const filled = Array.from({ length: filledIcons }, (_, index) => (
    ...
    const empty = Array.from(
      { length: maxRating - filledIcons - (remainder > 0 ? 1 : 0) },
      (_, index) => <FavoriteBorderIcon key={index} />
    );
    ...
    const enquiryToService = async () => {
      if (eventValue) {
        ...
        try {
          ...
          console.error(
            "Error fetching Enquiry Convert to Service Data:",
            error
          );
        }
      }
  ...
  useEffect(() => {
    const getExtendService = async () => {
      if (eventID) {
        try {
          const res = await fetch(
            `${port}/web/get_service_details/${eventID}/`,
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
                "Content-Type": "application/json",
          ...
          setsrvExtendSrvPurpId(data.data.purp_call_id);
          if (data.data.cc_id && data.data.cc_id.cc_id) {
            setSelectedCare(data.data.cc_id.cc_id);
          }
        } catch (error) {
  ...
  async function handleSubmit(event, actionType) {
    ...
    const requestData = {
      ...
      discount_value: selectedDiscountId === 1 ? couponValue : discountValue,
      // final_amount: totalDiscount,
      final_amount:
        selectedDiscountId === 3 ? 0 : totalDiscount || calculatedAmount,
    ...
    if (enqToServicePtn && enqToServicePay) {
      ...
      requestData.preferred_hosp_id = enqToServicePtn.preferred_hosp_id
        ? enqToServicePtn.preferred_hosp_id.hosp_id
        : selectedHospital;
      // requestData.Suffered_from = enqToServicePtn.Suffered_from;
      requestData.Suffered_from = enqToServicePay
        ? enqToServicePay.Suffered_from
        : suffered;
      requestData.phone_no = enqToServicePtn.phone_no;
      requestData.patient_email_id = enqToServicePtn.patient_email_id
        ? enqToServicePtn.patient_email_id
        : email;
      requestData.doct_cons_id = enqToServicePtn.doct_cons_id
        ? enqToServicePtn.doct_cons_id.doct_cons_id
        : selectedConsultant;
      requestData.doct_cons_phone = enqToServicePtn.doct_cons_id
        ? enqToServicePtn.doct_cons_id.mobile_no
        : consultantMobile;
      // requestData.state_id = enqToServicePtn.state_id.state_id;
      requestData.state_id = selectedState;
      requestData.city_id = enqToServicePtn.city_id
        ? enqToServicePtn.city_id.city_id
        : selectedCity;
      requestData.prof_zone_id = enqToServicePtn.prof_zone_id
        ? enqToServicePtn.prof_zone_id.prof_zone_id
        : selectedZone;
      ...
      requestData.google_address = enqToServicePtn
        ? enqToServicePtn.google_address
        : gisAddress;
      requestData.lattitude = enqToServicePtn.lattitude
        ? enqToServicePtn.lattitude
        : lat;
      requestData.langitude = enqToServicePtn.langitude
        ? enqToServicePtn.langitude
        : long;
      ...
      requestData.discount_type = enqToServicePay.discount_type
        ? enqToServicePay.discount_type
        : selectedDiscountId === 6
          ? 1
          : selectedDiscountId;
      requestData.discount_value = enqToServicePay.discount_value
        ? enqToServicePay.discount_value
        : discountValue;
      requestData.final_amount =
        selectedDiscountId === 3
          ? 0
          : enqToServicePay.final_amount
            ? enqToServicePay.final_amount
            : totalDiscount;
      ...
      requestData.preferred_hosp_id = srvExtendPtn.hosp_id
        ? srvExtendPtn.hosp_id.hosp_id
        : selectedHospital;
      requestData.Suffered_from = srvExtendPtn.Suffered_from;
      requestData.phone_no = srvExtendPtn.phone_no;
      requestData.patient_email_id = srvExtendPtn.patient_email_id
        ? srvExtendPtn.patient_email_id
        : email;
      requestData.doct_cons_id = srvExtendPtn.doct_cons_id
        ? srvExtendPtn.doct_cons_id.doct_cons_id
        : selectedConsultant;
      requestData.doct_cons_phone = srvExtendPtn.doct_cons_id
        ? srvExtendPtn.doct_cons_id.mobile_no
        : consultantMobile;
      requestData.state_id = selectedState;
      requestData.city_id = srvExtendPtn.city_id
        ? srvExtendPtn.city_id.city_id
        : selectedCity;
      requestData.prof_zone_id = srvExtendPtn.prof_zone_id
        ? srvExtendPtn.prof_zone_id.id
        : selectedZone;
      requestData.pincode = pincode;
      // requestData.google_address = srvExtendPtn.google_address;
      requestData.address = srvExtendPtn.address;
      // requestData.google_address = gisAddress;
      requestData.lattitude = srvExtendPtn.lattitude
        ? srvExtendPtn.lattitude
        : lat;
      requestData.langitude = srvExtendPtn.langitude
        ? srvExtendPtn.langitude
        : long;
      requestData.google_address = srvExtendPtn.google_address || gisAddress;
    ...
    if (enqToServiceSrv) {
      ...
      requestData.prof_prefered = enqToServiceSrv.prof_prefered
        ? enqToServiceSrv.prof_prefered
        : selectedProfGender;
      requestData.remark = enqToServiceSrv.remark
        ? enqToServiceSrv.remark
        : remark;
      requestData.lattitude = lat;
      requestData.langitude = long;
    } else if (srvExtendSrv) {
      ...
      requestData.Total_cost =
        selectedService === 12 ? medTransAmt * dateCount : calculatedAmount;
      requestData.discount_type =
        selectedDiscountId === 6 ? 1 : selectedDiscountId;
      // requestData.discount_value = discountValue;
      requestData.discount_value =
        selectedDiscountId === 6 ? couponValue : discountValue;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          ...
          result &&
          result["Service Created Event Code"] &&
          result["Service Created Event Code"].length > 0
        ) {
          setOpenSnackbar(true);
          ...
          const eventValue =
            result["Service Created Event Code"][0]["event_id"];
          const patientValue =
            result["Service Created Event Code"][1]["agg_sp_pt_id"];
          const callerValue =
            result["Service Created Event Code"][1]["caller_id"];
          const eventPlanValue =
            result["Service Created Event Code"][2]["event_plan_of_care_id"][0];
          ...
          const callerValue =
            result["Service Created Event Code"]?.[1]?.["caller_id"] || "";
          const eventPlanValue =
            result["Service Created Event Code"]?.[2]?.[
            "event_plan_of_care_id"
            ]?.[0] || "";

          console.log("Enquiry Success - Navigating to viewservice");
          console.log("event ID:", eventValue);
          console.log("patientID:", patientValue);
          console.log("callerID:", callerValue);
          console.log("eventPlanID:", eventPlanValue);
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)}>
        ...
        >
          ...
          >
            ...
            <BlockIcon sx={{ fontSize: 32, color: 'red' }} />
          </Box>

          <Typography variant="body1" align="center">
            <strong>Remark:</strong> {patientData?.blacklist_remark || 'No remark provided.'}
          </Typography>
                  ...
                  {callerDetails ? (
                    <Grid item lg={6} sm={6} xs={12}>
                      <TextField
                        ...
                        value={
                          callerDetails
                            ? callerDetails.caller_fullname
                            : clrName
                        }
                        onChange={(e) =>
                          callerDetails
                            ? handleFieldEdit("caller_fullname", e.target.value)
                    ...
                    <Grid item lg={6} sm={6} xs={12}>
                      <TextField
                        ...
                        value={
                          enqToServiceClr
                            ? enqToServiceClr.caller_fullname
                            : clrName
                        }
                        onChange={(e) =>
                          enqToServiceClr
                    ...
                    <Grid item lg={6} sm={6} xs={12}>
                      <TextField
                        ...
                        placeholder="First Name | Last Name *"
                        value={
                          srvExtendClr ? srvExtendClr.caller_fullname : clrName
                        }
                        onChange={(e) =>
                          srvExtendClr
                            ? handleFieldUpdate(
                    ...
                    <>
                      <Grid item lg={6} sm={6} xs={12}>
                        ...
                        >
                          {call.map((option) => (
                            <MenuItem
                              key={option.purp_call_id}
                              value={option.purp_call_id}
                              sx={{ fontSize: "14px" }}
                            >
                              {option.name}
                            </MenuItem>
                      ...
                      <Grid item lg={6} sm={6} xs={12}>
                        ...
                        >
                          {relation.map((option) => (
                            <MenuItem
                              key={option.caller_rel_id}
                              value={option.caller_rel_id}
                              sx={{ fontSize: "14px" }}
                            >
                              {option.relation}
                            </MenuItem>
                ...
                <Grid container spacing={2} sx={{ marginTop: "1px" }}>
                  {showAddPatient ? (
                    <>
                      ...
                      <Grid item lg={6} sm={6} xs={12}>
                        {isLoaded && (
                          <Autocomplete onLoad={(autocomplete) => (addressRef.current = autocomplete)} onPlaceChanged={handlePlaceChanged}>
                            <div item lg={6} sm={6} xs={12} style={{ width: '280px' }}>
                              <TextField
                                required
                                label="GIS Address"
                                id="google_address"
                        ...
                        <>
                            ...
                            <Grid container spacing={1}>
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  defaultValue={
                                    selectedPatient
                                      ? selectedPatient.gender.gender_id
                                      : selectedGender
                                  }
                                  onChange={(e) =>
                                    selectedPatient
                              ...
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  fullWidth
                                  defaultValue={
                                    selectedPatient ? selectedPatient.Age : age
                                  }
                                  onChange={(e) =>
                                    selectedPatient
                                      ? handleFieldEdit("Age", e.target.value)
                          ...
                          <Grid item lg={6} sm={6} xs={12}>
                            <TextField
                              ...
                              value={
                                selectedPatient.preferred_hosp
                                  ? selectedPatient.preferred_hosp.hosp_id
                                  : selectedHospital
                              }
                              onChange={(e) => {
                                const selectedHospitalId = e.target.value;
                                const selectedHospitalObj = referHospital.find(
                                  (option) =>
                                    option.hosp_id === selectedHospitalId
                                );
                                const selectedHospitalName = selectedHospitalObj
                                  ? selectedHospitalObj.hospital_name
                                  : "";
                                handleFieldEdit("preferred_hosp", {
                                  hosp_id: selectedHospitalId,
                            ...
                            <Grid container spacing={1}>
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  value={
                                    selectedPatient
                                      ? selectedPatient.phone_no.toString()
                                      : ptnNumber
                                  }
                                  onInput={(e) =>
                                    selectedPatient
                              ...
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  defaultValue={
                                    selectedPatient
                                      ? selectedPatient.patient_email_id
                                      : email
                                  }
                                  onChange={(e) =>
                                    selectedPatient
                            ...
                            <Grid container spacing={1}>
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  value={
                                    selectedPatient
                                      ? selectedPatient.doct_cons.doct_cons_id
                                      : selectedConsultant
                                  }
                                  onChange={(e) =>
                                    selectedPatient
                              ...
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  fullWidth
                                  value={
                                    selectedPatient.doct_cons
                                      ? selectedPatient.doct_cons.mobile_no
                                      : consultantMobile
                                  }
                                  // value={consultantMobile}
                                  sx={{
                            ...
                            <Grid container spacing={1}>
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  helperText={
                                    selectedPatient ? "" : errors.selectedState
                                  }
                                  sx={{
                                    textAlign: "left",
                                    "& input": {
                            ...
                            <Grid container spacing={1}>
                              <Grid item xs={6}>
                                <TextField
                                  ...
                                  defaultValue={
                                    selectedPatient
                                      ? selectedPatient.zone.prof_zone_id
                                      : selectedZone
                                  }
                                  onChange={(e) =>
                                    selectedPatient
                          ...
                          <Grid item lg={6} sm={6} xs={6}>
                            <TextField
                              ...
                              onChange={(e) =>
                                selectedPatient
                                  ? handleFieldEdit("address", e.target.value)
                    ...
                    <>
                      <Grid item lg={6} sm={6} xs={12}>
                        <TextField
                          ...
                          placeholder="First Name | Last Name "
                          value={
                            enqToServicePtn ? enqToServicePtn.name : ptnName
                          }
                          onChange={(e) => {
                            const inputName = e.target.value;
                            setPtnName(inputName);
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              value={
                                enqToServicePtn
                                  ? enqToServicePtn.gender_id.gender_id
                                  : selectedGender
                              }
                              onChange={(e) => {
                                const selectedGenderId = e.target.value;
                                const selectedGenderObj = gender.find(
                                  (option) =>
                                    option.gender_id === selectedGenderId
                                );
                                const selectedGenderName = selectedGenderObj
                                  ? selectedGenderObj.name
                                  : "";
                                handleFieldChange("gender_id", {
                                  gender_id: selectedGenderId,
                                  name: selectedGenderName,
                                });
                              }}
                              sx={{
                                textAlign: "left",
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              name="Age"
                              value={
                                enqToServicePtn ? enqToServicePtn.Age : age
                              }
                              onChange={(e) =>
                                enqToServicePtn
                      ...
                      <Grid item lg={6} sm={6} xs={12}>
                        <TextField
                          ...
                          fullWidth
                          value={
                            enqToServicePtn.preferred_hosp_id
                              ? enqToServicePtn.preferred_hosp_id.hosp_id
                              : selectedHospital
                          }
                          onChange={(e) => {
                            const selectedHospitalId = e.target.value;
                            const selectedHospitalObj = referHospital.find(
                              (option) => option.hosp_id === selectedHospitalId
                            );
                            const selectedHospitalName = selectedHospitalObj
                              ? selectedHospitalObj.hospital_name
                              : "";
                            handleFieldChange("preferred_hosp_id", {
                              hosp_id: selectedHospitalId,
                              hospital_name: selectedHospitalName,
                            });
                          }}
                          error={
                            (enqToServicePtn.preferred_hosp_id === null &&
                              !selectedHospital) ||
                            !!errors.hospital
                          }
                          helperText={
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              value={
                                enqToServicePtn
                                  ? enqToServicePtn.phone_no.toString()
                                  : ptnNumber
                              }
                              onInput={(e) =>
                                enqToServicePtn
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              error={
                                (enqToServicePtn.patient_email_id === null &&
                                  !email) ||
                                !!errors.email
                              }
                              helperText={
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              error={
                                (enqToServicePtn.doct_cons_id === null &&
                                  !selectedConsultant) ||
                                !!errors.selectedConsultant
                              }
                              helperText={
                                enqToServicePtn.doct_cons_id === null &&
                                  !selectedConsultant
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              helperText={
                                enqToServicePtn ? "" : errors.consultantMobile
                              }
                              inputProps={{
                                minLength: 10,
                                maxLength: 10,
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              error={
                                enqToServicePtn ? false : !!errors.selectedState
                              }
                              helperText={
                                enqToServicePtn ? "" : errors.selectedState
                              }
                              sx={{
                                textAlign: "left",
                                "& input": {
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              onChange={(e) => {
                                ...
                                const selectedCityObj = city.find(
                                  (option) => option.city_id === selectedCityId
                                );
                              ...
                              error={
                                enqToServicePtn ? false : !!errors.selectedCity
                              }
                              helperText={
                                enqToServicePtn ? "" : errors.selectedCity
                              }
                              sx={{
                                textAlign: "left",
                                "& input": {
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              onChange={(e) => {
                                ...
                                const selectedZoneObj = zone.find(
                                  (option) =>
                                    option.prof_zone_id === selectedZoneId
                                );
                                const selectedZoneName = selectedZoneObj
                                  ? selectedZoneObj.Name
                                  : "";

                                handleFieldChange("prof_zone_id", {
                                  prof_zone_id: selectedZoneId,
                                  Name: selectedZoneName,
                                });
                              }}
                              error={
                                (enqToServicePtn.prof_zone_id === null &&
                                  !selectedZone) ||
                                !!errors.selectedZone
                              }
                              helperText={
                      ...
                      {isLoaded && (
                        <Autocomplete onLoad={(autocomplete) => (addressRef.current = autocomplete)} onPlaceChanged={handlePlaceChanged}>
                          <div item lg={6} sm={6} xs={12} style={{ width: '280px', marginTop: '15px', marginLeft: '15px' }}>
                            <TextField
                              required
                              label="GIS Address"
                              id="google_address"
                      ...
                      <Grid item lg={6} sm={6} xs={6}>
                        <TextField
                          ...
                          fullWidth
                          value={
                            enqToServicePtn ? enqToServicePtn.address : address
                          }
                          onChange={(e) =>
                            enqToServicePtn
                    ...
                    <>
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              value={
                                srvExtendPtn
                                  ? srvExtendPtn.gender_id.gender_id
                                  : selectedGender
                              }
                              onChange={(e) => {
                                const selectedGenderId = e.target.value;
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              onChange={(e) =>
                                srvExtendPtn
                                  ? handleFieldUpdate("Age", e.target.value)
                                  : handleAgeValidation(e)
                              }
                              size="small"
                              fullWidth
                      ...
                      <Grid item lg={6} sm={6} xs={12}>
                        <TextField
                          ...
                          placeholder="Remark"
                          value={
                            srvExtendPtn ? srvExtendPtn.Suffered_from : suffered
                          }
                          onChange={(e) =>
                            srvExtendPtn
                              ? handleFieldUpdate(
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              value={
                                srvExtendPtn
                                  ? srvExtendPtn.phone_no.toString()
                                  : ptnNumber
                              }
                              onInput={(e) =>
                                srvExtendPtn
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              helperText={
                                srvExtendPtn ? "" : errors.selectedConsultant
                              }
                              sx={{
                                textAlign: "left",
                                "& input": {
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              error={
                                srvExtendPtn ? false : !!errors.consultantMobile
                              }
                              helperText={
                                srvExtendPtn ? "" : errors.consultantMobile
                              }
                              inputProps={{
                                minLength: 10,
                                maxLength: 10,
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              onChange={(e) => setSelectedState(e.target.value)}
                              error={
                                srvExtendPtn ? false : !!errors.selectedState
                              }
                              helperText={
                                srvExtendPtn ? "" : errors.selectedState
                              }
                              sx={{
                                textAlign: "left",
                                "& input": {
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              onChange={(e) => {
                                ...
                                handleFieldUpdate("city_id", {
                                  city_id: selectedCityId,
                                  city_name: selectedCityName,
                                });
                              }}
                              error={
                                srvExtendPtn ? false : !!errors.selectedCity
                              }
                              helperText={
                                srvExtendPtn ? "" : errors.selectedCity
                              }
                              sx={{
                                textAlign: "left",
                                "& input": {
                        ...
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            <TextField
                              ...
                              onChange={(e) => {
                                ...
                                handleFieldUpdate("prof_zone_id", {
                                  id: selectedZoneId,
                                  Name: selectedZoneName,
                                });
                              }}
                              ...
                              error={
                                srvExtendPtn ? false : !!errors.selectedZone
                              }
                              helperText={
                                srvExtendPtn ? "" : errors.selectedZone
                              }
                              sx={{
                                textAlign: "left",
                                "& input": {
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              fullWidth
                              value={
                                srvExtendPtn ? srvExtendPtn.pincode : pincode
                              }
                              onChange={handlePincodeChange}
                              inputProps={{
                                minLength: 6,
                      ...
                      <Grid item lg={12} sm={12} xs={12}>
                        <TextField
                          ...
                          onChange={(e) => {
                            ...
                            const validationError = validateFullName(inputName);
                            setConsultErrors((prevErrors) => ({
                              ...prevErrors,
                              consultName: validationError,
                            }));
                          }}
                          sx={{
                            "& input": {
                    ...
                    <>
                      <Grid item lg={12} sm={12} xs={12}>
                        <TextField
                          ...
                          fullWidth
                          error={
                            enqToServiceSrv ? false : !!errors.selectedService
                          }
                          helperText={
                            enqToServiceSrv ? "" : errors.selectedService
                          }
                          sx={{
                            textAlign: "left",
                            "& input": {
                      ...
                      >
                        ...
                        >
                          <div style={{ display: "flex" }}>
                            <Typography
                              sx={{ pl: 2, fontSize: 18, fontWeight: 600 }}
                        ...
                        <DatePicker
                          ...
                          render={(value, openCalendar) => (
                            <TextField
                              ...
                              value={selectedDates}
                              error={
                                enqToServiceSrv ? false : !!errors.serivce_dates
                              }
                              helperText={
                                enqToServiceSrv ? "" : errors.serivce_dates
                              }
                              sx={{
                                textAlign: "left",
                                "& input": {
                      ...
                      <Grid container spacing={1} sx={{ mt: 2, ml: 1 }}>
                        ...
                        <Grid item lg={6} sm={6} xs={6}>
                          <TextField
                            ...
                            error={
                              (enqToServiceSrv.end_time === null && !endTime) ||
                              !!errors.endTime
                            }
                            helperText={
                              enqToServiceSrv.end_time === null && !endTime
                      ...
                      <Grid item lg={12} sm={12} xs={12}>
                        <TextField
                          ...
                          fullWidth
                          value={
                            enqToServiceSrv ? enqToServiceSrv.remark : remark
                          }
                          onChange={(e) =>
                            enqToServiceSrv
                    ...
                    <>
                      <Grid item lg={12} sm={12} xs={12}>
                        <TextField
                          ...
                          fullWidth
                          error={
                            srvExtendSrv ? false : !!errors.selectedService
                          }
                          helperText={
                            srvExtendSrv ? "" : errors.selectedService
                          }
                          sx={{
                            textAlign: "left",
                            "& input": {
                    ...
                    <Grid container spacing={1}>
                      ...
                      >
                        ...
                        >
                          {discount.map((option) => (
                            <MenuItem
                              key={option.discount_id}
                              value={option.discount_id}
                            >
                              {option.label}
                            </MenuItem>
                          ))}
                      ...
                      <Grid item xs={6}>
                        {selectedDiscountId === 1 && (
                          <TextField
                            ...
                            helperText={
                              !isDiscountValueValid() && "This is required"
                            }
                            sx={{
                              textAlign: "left",
                              "& input": {
                        ...
                        {selectedDiscountId === 6 && (
                          ...
                          >
                            {coupon.map((option) => (
                              <MenuItem
                                key={option.coupon_id}
                                value={option.coupon_id}
                              >
                                {option.Code}
                              </MenuItem>
                            ))}
        ...
        <Grid>
          {selectedCall !== 2 && (
            <Button
              ...
              onClick={(event) => handleSubmit(event, "CreateService")}
            >
              Request Service
            </Button>
          )}
