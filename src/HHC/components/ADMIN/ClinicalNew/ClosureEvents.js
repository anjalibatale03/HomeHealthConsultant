...
const ClosureEvents = ({
  ...
  useEffect(() => {
    console.log(events, "eeeee");
  }, [events]);

  const jobClosureDtl = events[0]?.job_closure_dtl || [];
  const hca_jc =
    (events[0]?.job_closure_dtl && events[0].job_closure_dtl[0]?.hca_jc) || [];
  ...
  const [dtlEvtId, setDtlEvtId] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  //form1
  const [remarks, setRemarks] = useState(Array(jobClosureDtl.length).fill(""));
  //form one
  const [baselineState, setBaseline] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [airwayState, setAirway] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [breathingState, setBreathing] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  ...
  //form ten
  const [Strength_exer, setStrength_exer] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [Stretch_exer, setStretch_exer] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [Walk_indep, setWalk_indep] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [Walker_stick, setWalker_stick] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  ...
  // form four
  const [Nameinjectionfld, setNameinjectionfld] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [InjsiteIM, setInjsiteIM] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [Dosefreq, setDosefreq] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [Remark4, setRemark4] = useState(Array(jobClosureDtl.length).fill(""));

  // form Five
  const [Catheter_type, setCatheter_type] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  const [Size_catheter, setSize_catheter] = useState(
    Array(jobClosureDtl.length).fill("")
  );
  ...
  useEffect(() => {
    ...
    if (jobClosureDtl) {
      const previousRemarks = jobClosureDtl.map(
      ...
      const skinState = jobClosureDtl.map(
        (detail) => detail.Skin_Perfusion || ""
      );
      ...
      // f10
      const strengthexer = jobClosureDtl.map(
        (detail) => detail.Strength_exer || ""
      );
      const stretchexer = jobClosureDtl.map(
        (detail) => detail.Stretch_exer || ""
      );
      const walkindep = jobClosureDtl.map((detail) => detail.Walk_indep || "");
      const walkerstick = jobClosureDtl.map(
        (detail) => detail.Walker_stick || ""
      );
      ...
      const dressing1 = jobClosureDtl.map(
        (detail) => detail.Dressing_status || ""
      );

      // f3
      const Nameinjectionfld1 = jobClosureDtl.map(
        (detail) => detail.Name_injection_fld || ""
      );
      const InjsiteIM1 = jobClosureDtl.map(
        (detail) => detail.Inj_site_IM || ""
      );
      const Dosefreq1 = jobClosureDtl.map((detail) => detail.Dose_freq || "");
      const remark1 = jobClosureDtl.map((detail) => detail.Remark || "");

      //f5
      const Cathetertype = jobClosureDtl.map(
        (detail) => detail.Catheter_type || ""
      );
      const Sizecatheter = jobClosureDtl.map(
        (detail) => detail.Size_catheter || ""
      );
      ...
      const staples1 = jobClosureDtl.map(
        (detail) => detail.Num_Sutures_staples || ""
      );
      const dehiWound1 = jobClosureDtl.map(
        (detail) => detail.Wound_dehiscence || ""
      );
  ...
  const [expandedFlags, setExpandedFlags] = useState(
    Array(jobClosureDtl.length).fill(false)
  );
  ...
  return (
    <Box>
      <Grid container spacing={1}>
        <Grid item md={3} xs={12} style={{ position: "fixed", width: "70%" }}>
        ...
        >
          ...
          >
            <div>
              ...
              <Grid item>
                <Typography textAlign={"left"} sx={{ marginBottom: "2px" }}>
                  Suffered From: {events[0]?.Suffered_from || ""}
                </Typography>
                <Grid container spacing={2}>
                  <Grid item md={6} xs={12}>
                    <Typography variant="body2" sx={{ color: "gray" }}>
                      Consultant Name:{" "}
                      {events[0]?.pt_dtt?.doct_cons_id?.cons_fullname || ""}
                    </Typography>
                  </Grid>
                  ...
                  <Grid item md={6} xs={12}>
                    <Typography variant="body2" sx={{ color: "gray" }}>
                      Consultant Phone:{" "}
                      {events[0]?.pt_dtt?.doct_cons_id?.mobile_no || ""}
                    </Typography>
                  </Grid>
                </Grid>
              </Grid>
              {jobClosureDtl.length === 0 ? (
                <Card
                  style={{
                    background: "#f9f9f9",
                    boxShadow: "4px 4px 10px 7px rgba(135, 135, 135, 0.1)",
                    borderRadius: "10px",
                    padding: "20px",
                    textAlign: "center",
                    marginTop: "20px",
                    ...
                    <AccordionDetails>
                      <Grid container>
                        ...
                        <Grid item md={6} xs={12}>
                          <Typography variant="p" sx={{ color: "gray" }}>
                            Professional Phone:{" "}
                            {detail?.prof_name?.prof_no || ""}
                          </Typography>
                          <br />
                        </Grid>
                        <Grid item md={6} xs={12}>
                          <Typography variant="p" sx={{ color: "gray" }}>
                            Sub Service Name: {detail?.sub_srv_name || ""}
                          </Typography>
                          <br />
                        </Grid>
                        <Grid item md={6} xs={12}>
                      ...
                      <Typography>
                        {events[0]?.job_cl_fm_no == 1 && (
                          <ClosureFormOne
                            ...
                            closureStatus={
                              events[0]?.job_closure_dtl[0].is_job_closure_done
                            }
                            dtlEventID={
                              events[0]?.job_closure_dtl[0].dtl_eve_id
                            }
                            handleEvent={handleEvent}
                            eid={eid}
                            isViewOnly={true} // 👈 Add this
                          />
                        )}

                        {events[0]?.job_cl_fm_no == 2 && (
                          <ClosureFormtwo
                            events={hca_jc}
                            evtId={dtlEvtId[index]}
                            formNo={events[0]?.job_cl_fm_no}
