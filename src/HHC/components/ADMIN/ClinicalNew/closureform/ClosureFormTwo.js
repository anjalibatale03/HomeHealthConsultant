...
const ClosureFormtwo = ({
  ...
  const initializeRemarks = () =>
    events.map((item) => {
      if (item.remark_type === "Remark") {
        const remarkKey = Object.keys(item).find((key) =>
          key.endsWith("_datetime_remark")
        );
  ...
  const initializeRadioValues = () =>
    events.map((item) => (item.remark_type === "Remark" ? "1" : "2"));
    ...
    try {
      if (closureStatus == true) {
        const response = await fetch(
          `${port}/medical/update_job_closure_form_api/${evtId}/`,
          {
            method: "PUT",
            headers: {
        ...
        const response = await fetch(
          `${port}/medical/Medical_job_closure/?dtl_eve=${dtlEventID}`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <>
      ...
      >
        <Grid item xs={12} container spacing={1} sx={{ mt: 1 }}>
          {events &&
            events.map((item, index) => {
              ...
              const thirdKey = Object.keys(item).find((key) =>
                key.endsWith("_datetime_remark")
              );
                    ...
                    >
                      <FormControl disabled>
                        ...
                        <RadioGroup
                          ...
                          onChange={(e) =>
                            handleOptionChange(e, index, secondKey)
                          }
                        >
                          <FormControlLabel
                            value="1"
                      ...
                      {selectedOptions[index] === "2" && (
                        <TextField
                          ...
                          value={remarks[index] || ""}
                          onChange={(e) =>
                            handleRemarkChange(e, index, thirdKey)
                          }
                          sx={{ mt: 1 }}
                          multiline
                          rows={2}
                      ...
                      {selectedOptions[index] === "1" && (
                        <TextField
                          ...
                          onChange={(e) =>
                            handleDateTimeChange(e, index, thirdKey)
                          }
                        />
                      )}
                    </Box>
          ...
          <Grid item xs={12} sm={12} md={6} lg={6}>
            {/* <TextField
              ...
              helperText={
                submitAttempted && !medGovRemark.trim()
                  ? "Medical Governance Remark is required"
                  : ""
              }
            /> */}
          </Grid>
        ...
        <Grid item xs={12} sm={12} lg={12} sx={{ mt: 1, ml: "20rem", mb: 4 }}>
          {/* <Button
            variant="contained"
            sx={{
              width: "20ch",
