...
const ManageService = () => {
  ...
  const fetchSubServices = async (selectedService) => {
    ...
    try {
      const response = await fetch(
        `${port}/hhc_hcm/manage_sub_srv/?srv=${selectedService}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  const handleSubServiceForm = () => {
    ...
    if (!subService.recommomded_service) {
      newErrorMessagesSub.recommomded_service =
        "Recommomded Service is required.";
    }
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        if (method === "POST") {
          setSubServices((prevSubServices) => [...prevSubServices, data]);
        } else {
          fetchSubServices(selectedService.srv_id);
          // setSubServices((prevSubServices) =>
          //   prevSubServices.map((sub) =>
          //     sub.sub_srv_id === data.sub_srv_id ? data : sub
          //   )
          // );
        }
        // fetchSubServices(selectedService.srv_id);
  ...
  const handleStatusIconClick = async (srvId, status) => {
    const confirmMessage =
      status === 1
        ? "Are you sure you want to Inactivate the service?"
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        const accessToken = localStorage.getItem("token");
        const response = await fetch(
          `${port}/hhc_hcm/service_inactive_hhc/${srvId}/${newStatus}/`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${accessToken}`,
        ...
        if (response.ok) {
          ...
          setService((prevService) =>
            prevService.map((item) =>
              item.srv_id === srvId ? { ...item, status: newStatus } : item
            )
          );
  ...
  const handleSubServiceIcon = async (subsrvId, status) => {
    const confirmMessage =
      status === 1
        ? "Are you sure you want to Inactivate the sub service?"
        : "Are you sure you want to Activate the sub service?";
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        const accessToken = localStorage.getItem("token");
        const response = await fetch(
          `${port}/hhc_hcm/sub_service_inactive_hhc/${subsrvId}/${newStatus}/`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${accessToken}`,
                    ...
                    <ManageServiceCard>
                      ...
                      <CardContent style={{ flex: 1 }}>
                        <Tooltip title="Add Sub Service" arrow>
                          <IconButton
                            aria-label="add"
                            onClick={() => handleSubServiceClick(item)}
                          >
                          ...
                          <IconButton
                            ...
                            onClick={() =>
                              handleStatusIconClick(
                                item.srv_id,
                                serviceStatus[item.srv_id] || item.status
                              )
                            }
                          >
                            {serviceStatus[item.srv_id] === 1 ||
                              item.status === 1 ? (
                        ...
                        >
                          ...
                          <Tooltip title="Status" arrow>
                            ...
                            <IconButton
                              aria-label="edit"
                              onClick={() =>
                                handleSubServiceIcon(
                                  item.sub_srv_id,
                                  serviceStatus[item.sub_srv_id] || item.status
                                )
                              }
                            >
                              {serviceStatus[item.sub_srv_id] === 1 ||
                                item.status === 1 ? (
                                <CheckIcon style={{ color: "green" }} />
                              ) : (
                                <ClearIcon style={{ color: "red" }} />
