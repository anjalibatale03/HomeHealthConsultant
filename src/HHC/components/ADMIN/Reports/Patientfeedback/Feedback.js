...
const Feedback = ({ eveID, ptnid }) => {
    ...
    const fetchProfessionalData = async () => {
      try {
        const response = await fetch(
          `${port}/web/Prof_names_eve_wise/${eveID}`,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch(
          `${port}/web/feedback_questions/?lang=eng`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
  ...
  }, [accessToken, eveID, ptnid, port]);

  const fetchFeedbackData = async () => {
    try {
      const response = await fetch(
        `${port}/web/save_feedback_q/?eve_id=${eveID}&ptn_id=${ptnid}&prof_id=${selectedDropdownValue}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  const validateFields = () => {
    ...
    questions.forEach((question) => {
      if ([16, 17, 18].includes(question.F_questions)) {
        const answer =
          selectedAnswers[question.F_questions] ||
          textInputs[question.F_questions];
        if (!answer) {
  ...
  const disabledAll = formSubmitted || feedbackDataState.length > 0;
  return (
        ...
        {tabValue === "patient" && (
          <Grid container spacing={2} sx={{ marginTop: "1px" }}>
            ...
            {questions.map((question, index) => (
              <Grid key={index} item lg={12} sm={12} xs={12}>
                <Typography variant="h6" style={{ fontSize: "16px" }}>
                  {question.question}
                  {[16, 17, 18].includes(question.F_questions) && (
                    ...
                    <Typography variant="body1" style={{ fontSize: "15px" }}>
                      <Rating
                        ...
                        onChange={(event, newValue) =>
                          handleRatingChange(question.F_questions, newValue)
                        }
                        disabled={disabledAll}
                      />
                    </Typography>
            ...
            <Grid item lg={12} sm={12} xs={12} style={{ textAlign: "center" }}>
              {(formSubmitted || feedbackDataState.length > 0) && (
                <Typography
                  variant="h6"
                  sx={{
