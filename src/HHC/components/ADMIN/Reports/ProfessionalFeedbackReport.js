...
function ProfessionalFeedbackReport() {
...
const getPlaceholderText = () => {
 ...
 const handleDownload = async () => {
  ...
  let query = `start_date=${formattedStart}&end_date=${formattedEnd}`;
  ...
  try {
    const response = await fetch(
      `${port}/web/downloadprofessional_feedback/?${query}`,
      {
        headers: {
    ...
    if (response.status === 200) {
      ...
      link.href = url;
      link.download = `Professional_Feedback_Report_${formattedStart}_to_${formattedEnd}.xlsx`;
  ...
  const handleSubmit = async () => {
    if (!startDate || !endDate) {
      triggerSnackbar("Please select a date range.", "warning");
      return;
    }
    ...
    try {
      const response = await fetch(`${port}/web/professional_feedback/?${query}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  const filteredData = useMemo(
    () =>
      professionaldata.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage),
    [professionaldata, page, rowsPerPage]
  );
  ...
  spacing={2}
  alignItems={
    isSmallScreen || isTablet ? "center" : "flex-start"
  }
  justifyContent={
    isSmallScreen || isTablet ? "center" : "flex-start"
  }
...
>
  <TextField
    ...
    sx={{
      "& .MuiSelect-select": {
        padding: "8px", // dropdown ke andar spacing adjust
      },
      "& fieldset": {
        border: "none", // agar border double aa rahi ho toh
      },
    }}
  >
  ...
  >
    <DownloadIcon />
        ...
        <TableContainer
          sx={{ height: professionaldata.length < 5 ? "60vh" : "auto" }}
        >
          <Table>
            <TableHead>
              <TableRow>
                  ...
                  <TableRow key={index}>
                    <TableCell colSpan={6} sx={{ border: "none", p: 0 }}>
                      ...
                      >
                        ...
                        <CardContent
                          sx={{ flex: 1.2, textAlign: "center", py: 0.5, px: 1 }}
                        >
                          {row.patient_phone || "N/A"}
                        </CardContent>
                        <CardContent
                          sx={{ flex: 1.5, textAlign: "center", py: 0.5, px: 1 }}
                        >
                          {row.start_date || "N/A"}
                        </CardContent>
                        <CardContent
...
maxHeight: "90vh",
  ...
  >
    ...
    >
      ...
      <IconButton onClick={handleCloseQestions} sx={{ color: "#FFF", p: 0.5 }}>
        <GridCloseIcon />
      </IconButton>
    </Box>
...
<Box
  sx={{
    maxHeight: "60vh", // ⬅ max height for scroll
    overflowY: "auto",
    px: 3,
    py: 3,
    bgcolor: "#f9f9f9",
...
>
  {selectedRow?.questions?.length > 0 ? (
    selectedRow.questions.map((qa, index) => (
      <Box key={index} sx={{ mb: 2 }}>
        ...
        {index !== selectedRow.questions.length - 1 && <Divider sx={{ my: 2 }} />}
      </Box>
    ))
  ) : (
    <Typography
...
export default ProfessionalFeedbackReport;
