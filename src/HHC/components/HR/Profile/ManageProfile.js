...
const ManageProfile = () => {
  ...
  const handleInterviewStatusOpen = async (profId) => {
    ...
    try {
      const response = await fetch(
        `${port}/hr/prof_interview_round/${profId}/`,
        {
          method: "GET",
          headers: {
  ...
  const handleInterviewSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${port}/hr/prof_interview_round/${profileId}/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    permission.modules_submodule.some((module) =>
      module.modules.some(
        (submodule) =>
          ...
          submodule.submodules.some(
            (sub) => sub.submodule_name === "Add New Professional"
          )
      )
    )
  );
    ...
    permission.modules_submodule.some((module) =>
      module.modules.some(
        (submodule) =>
          submodule.submodules &&
          submodule.submodules.some(
            (sub) => sub.submodule_name === "View Professional"
          )
      )
    )
  );
      ...
      <Box sx={{ flexGrow: 1, mt: 1, ml: 1, mr: 1, mb: 4 }}>
        ...
        >
          ...
          >
            <IconButton
              type="button"
              sx={{ color: "#69A5EB", height: "36px", width: "36px" }}
            >
              <SearchIcon />
            </IconButton>
            <InputBase
          ...
          {isAddServiceAllowed && (
            <Link
              to="/hr/manage profiles/add-prof"
            ...
            <TableHead>
              <TableRow>
                ...
                >
                  ...
                  <CardContent
                    style={{ flex: 1.8, borderRight: "1px solid #FFFFFF" }}
                  >
                    ...
                    .map((row, index) => (
                      ...
                      >
                        <ProfileCard>
                          ...
                          <CardContent style={{ flex: 1 }}>
                            <Typography variant="body2">
                              {row.manage_Profiles_status === 1
                                ? "Select"
                                : row.manage_Profiles_status === 2
                                  ? "Reject"
                                  : row.manage_Profiles_status === 3
                                    ? "On Hold"
                                    : row.manage_Profiles_status === 4
                                      ? "Shortlisted"
                                      : "-"}
                            </Typography>
                          </CardContent>
                          ...
                          <CardContent style={{ flex: 1 }}>
                            <Typography variant="body2">
                              {row.prof_zone_id ? row.prof_zone_id : "-"}
                            </Typography>
                          </CardContent>
                                ...
                                <Tooltip title="View Service">
                                  <Typography variant="body2">
                                    <RemoveRedEyeOutlinedIcon
                                      sx={{ fontSize: "26px", mt: 2 }}
                                      onClick={() =>
                                        handleIconClick(row.srv_prof_id)
                                      }
                                    />
                                  </Typography>
                                </Tooltip>
                              ...
                              <Tooltip title="Interview Status">
                                <Typography variant="body2">
                                  <NoteAddOutlinedIcon
                                    sx={{ fontSize: "26px", mt: 2, ml: 2 }}
                                    onClick={() =>
                                      handleInterviewStatusOpen(row.srv_prof_id)
                                    }
                                  />
                                </Typography>
                              </Tooltip>
                ...
                <>
                  <Grid item xs={12}>
                    ...
                    >
                      {intRound.map((option) => (
                        <MenuItem
                          key={option.intRound_id}
                          value={option.intRound_id}
                          sx={{ fontSize: "14px" }}
                        >
                          {option.label}
                        </MenuItem>
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      {intMode.map((option) => (
                        <MenuItem
                          key={option.intMode_id}
                          value={option.intMode_id}
                          sx={{ fontSize: "14px" }}
                        >
                          {option.label}
                        </MenuItem>
